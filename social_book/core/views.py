...
def index(request):
    ...
    return render(request, 'index.html', {'user_profile': user_profile, 'feed_list': feed_list,'suggestions_username_profile_list':suggestions_username_profile_list[:4]})
    ...
    if request.method == 'POST':
        ...
        if password == password2:
            if User.objects.filter(email=email).exists():
                ...
                messages.info(request, 'Username Alraedy Taken')
                return redirect('signup')
            else:
